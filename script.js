...
conversionBtn.addEventListener("click", function(){
    ...
    lengthEl.textContent = `${inputValue} meter = ${metToFeet.toFixed(3)} feet | ${inputValue} feet = ${feetToMet.toFixed(3)} meters` 
    ...
    volumeEl.textContent = `${inputValue} liters = ${litToGallon.toFixed(3)} gallons | ${inputValue} gallons = ${gallonToLit.toFixed(3)} liters`
    ...
    massEL.textContent = `${inputValue} kilogram = ${kiloToPound.toFixed(3)} pounds | ${inputValue} pounds = ${PoundToKilo.toFixed(3)} kilograms`
})
